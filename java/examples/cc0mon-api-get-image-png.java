...
class GetImagePng {
    public static void main(String[] args) throws Exception {
        Logger log = LoggingSetup.configure("get-image-png");
        Integer id = null;
        Path out = null;
        ...
        if (id == null) { System.err.println("usage: cc0mon-api-get-image-png.java --id <1..10000> [--out path]"); System.exit(5); }
        if (out == null) out = Paths.get(System.getProperty("user.dir"), "cc0mon-" + id + ".png");
        log.info("script start id=" + id + " out=" + out);

        try (Client client = new Client()) {
            byte[] bytes = client.getImagePng(id);
