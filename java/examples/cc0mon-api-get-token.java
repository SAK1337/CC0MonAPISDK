...
class GetToken {
    public static void main(String[] args) {
        Logger log = LoggingSetup.configure("get-token");
        Integer id = parseId(args);
        if (id == null) { System.err.println("usage: cc0mon-api-get-token.java --id <1..10000>"); System.exit(5); }
        log.info("script start id=" + id);
        try (Client client = new Client()) {
            Token token = client.getToken(id);
            System.out.println(new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT).writeValueAsString(token));
