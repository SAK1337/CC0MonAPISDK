...
class GetMetadata {
    public static void main(String[] args) {
        Logger log = LoggingSetup.configure("get-metadata");
        Integer id = parseId(args);
        if (id == null) { System.err.println("usage: cc0mon-api-get-metadata.java --id <1..10000>"); System.exit(5); }
        log.info("script start id=" + id);
        try (Client client = new Client()) {
            Metadata md = client.getMetadata(id);
            System.out.println(new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT).writeValueAsString(md));
            log.info("script exit code=0");
