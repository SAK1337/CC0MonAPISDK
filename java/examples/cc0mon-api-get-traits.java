...
class GetTraits {
    public static void main(String[] args) {
        Logger log = LoggingSetup.configure("get-traits");
        Integer id = parseId(args);
        if (id == null) { System.err.println("usage: cc0mon-api-get-traits.java --id <1..10000>"); System.exit(5); }
        log.info("script start id=" + id);
        try (Client client = new Client()) {
            Traits t = client.getTraits(id);
            System.out.println(new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT).writeValueAsString(t));
            log.info("script exit code=0 attributes=" + t.attributes().size());
